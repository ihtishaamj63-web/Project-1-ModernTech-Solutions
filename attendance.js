...
function initAttendance() {
  ...
  function buildEmployees() {
    return employeeInfo.map((emp) => {
      ...
      const todayRecord = attData
        ? attData.attendance.find((a) => a.date === today)
        : null;
  ...
  if (savedData) {
    employees = employees.map((emp) => {
      const savedLeave = savedData.leaveRequests?.find(
        (l) => l.employeeId === emp.id,
      );
      const savedAtt = savedData.attendance?.find(
  ...
  function getWeekSummary(emp) {
    ...
    return {
      ...
      present,
      absent: 5 - present,
      hasRecords: emp.attendance && emp.attendance.length > 0,
    };
  }

  function getAbsenceDetails(emp) {
    return getWeekRecords(emp)
  ...
  function calculateSummaryStats() {
    ...
    employees.forEach((emp) => {
      ...
      totalAbsent += absent;
      totalDays += 5;
      totalLeave += emp.leaveRequests.filter(
        (r) =>
          r.status === "Approved" &&
          dayLabels.some((d) => dateMapping[d] === r.date),
      ).length;
    });
    return {
      totalPresent,
  ...
  function updateSummaryCards() {
    ...
    document.getElementById("attAttendanceRate").textContent =
  ...
  function showEmployeeDetails(employeeId) {
    ...
    let html = `
      <div class="att-detail-section">
        ...
        <div class="att-detail-grid">
          <div class="att-detail-item"><span class="att-detail-label">Name</span><span class="att-detail-value">${emp.name}</span></div>
          <div class="att-detail-item"><span class="att-detail-label">Department</span><span class="att-detail-value">${emp.department}</span></div>
          <div class="att-detail-item"><span class="att-detail-label">Position</span><span class="att-detail-value">${emp.position}</span></div>
          <div class="att-detail-item"><span class="att-detail-label">Today's Status</span><span class="att-detail-value ${emp.status === "Active" ? "present" : emp.status === "On Leave" ? "absent" : ""}">${emp.status}</span></div>
        </div>
      </div>`;
          ...
          const cls =
            r.status === "Present"
              ...
              : r.status === "Absent"
                ? "absent"
                : "";
          return `<div class="att-detail-item"><span class="att-detail-label">${r.day}</span><span class="att-detail-value ${cls}">${status}</span></div>`;
        })
        .join("");
    ...
    if (absences.length) {
      html += `<div class="att-detail-section"><div class="att-detail-section-title"><i class="bi bi-exclamation-triangle"></i> Absence Details</div><div class="att-detail-grid">${absences.map((a) => `<div class="att-detail-item"><span class="att-detail-label">${a.day} (${a.date})</span><span class="att-detail-value absent">${a.reason} <span class="att-status-chip ${a.status.toLowerCase()}">${a.status}</span></span></div>`).join("")}</div></div>`;
    }

    if (emp.leaveRequests.length) {
      html += `<div class="att-detail-section"><div class="att-detail-section-title"><i class="bi bi-clock-history"></i> Leave Requests</div><div class="att-detail-grid">${emp.leaveRequests.map((req) => `<div class="att-detail-item"><span class="att-detail-label">${req.date}</span><span class="att-detail-value">${req.reason} <span class="att-status-chip ${req.status.toLowerCase()}">${req.status}</span></span></div>`).join("")}</div></div>`;
    }
  ...
  function renderWeeklyChart() {
    ...
    const gridHtml = gridLines
      .map(
        (value) =>
          `<div class="att-chart-grid-line" style="top: ${100 - (value / maxValue) * 100}%;"><span class="att-grid-label">${Math.round(value)}</span></div>`,
      )
      .join("");
    const barSegment = (type, value, pixelHeight) => {
      if (value <= 0) return "";
      return `<div class="att-chart-bar ${type}" style="height:${Math.max(pixelHeight, 4)}px" data-day-value="${value}" data-type="${type}"></div>`;
    };
    const dayColumns = weeklyData.days
      .map((day, i) => {
        const presentVal = weeklyData.present[i] || 0,
          leaveVal = weeklyData.leave[i] || 0,
          absentVal = weeklyData.absent[i] || 0;
        const dayTotal = presentVal + leaveVal + absentVal;
        const totalPixelHeight =
          dayTotal > 0
            ? (Math.min((dayTotal / maxValue) * 100, 100) / 100) * chartHeight
            : 0;
        const presentPx = dayTotal
          ? (presentVal / dayTotal) * totalPixelHeight
          : 0;
        const leavePx = dayTotal ? (leaveVal / dayTotal) * totalPixelHeight : 0;
        const absentPx = dayTotal
          ? (absentVal / dayTotal) * totalPixelHeight
          : 0;
        const bars =
          dayTotal > 0
            ? barSegment("present", presentVal, presentPx) +
              barSegment("leave", leaveVal, leavePx) +
              barSegment("absent", absentVal, absentPx)
            : `<div class="att-chart-bar" style="height:6px;background:var(--border-color);cursor:default;"></div>`;
    ...
    const attendanceRate =
      grandTotal > 0 ? ((totals.present / grandTotal) * 100).toFixed(1) : 0;

    container.innerHTML = `
      <div class="att-chart-wrapper"><div class="att-chart-grid">${gridHtml}</div><div class="att-chart-container">${dayColumns}</div><div class="att-chart-tooltip" id="attChartTooltip"></div></div>
      ...
      <div class="att-chart-stats">
        <div class="att-chart-stat"><i class="bi bi-bar-chart-line att-stat-icon"></i><span class="att-stat-label">Total Records:</span><span class="att-stat-value">${grandTotal}</span></div>
        ...
        <div class="att-chart-stat"><i class="bi bi-graph-up-arrow att-stat-icon"></i><span class="att-stat-label">Best Day:</span><span class="att-stat-value">${getBestDay()}</span></div>
        <div class="att-chart-stat"><i class="bi bi-graph-down-arrow att-stat-icon"></i><span class="att-stat-label">Worst Day:</span><span class="att-stat-value">${getWorstDay()}</span></div>
      </div>`;
  ...
  function bindChartTooltip(container) {
    const tooltip = container.querySelector("#attChartTooltip");
    const wrapper = container.querySelector(".att-chart-wrapper");
    container.querySelectorAll(".att-chart-bar[data-type]").forEach((bar) => {
      bar.addEventListener("mouseenter", function () {
        tooltip.textContent = `${this.dataset.type.charAt(0).toUpperCase() + this.dataset.type.slice(1)}: ${this.dataset.dayValue}`;
        tooltip.style.display = "block";
        const barRect = bar.getBoundingClientRect(),
          wrapperRect = wrapper.getBoundingClientRect();
        tooltip.style.left = `${barRect.left - wrapperRect.left + barRect.width / 2}px`;
        tooltip.style.top = `${barRect.top - wrapperRect.top - 8}px`;
      });
  ...
  function getBestDay() {
    ...
    dayLabels.forEach((_, i) => {
      const total =
        (weeklyData.present[i] || 0) +
        (weeklyData.leave[i] || 0) +
        (weeklyData.absent[i] || 0);
      const rate = total > 0 ? ((weeklyData.present[i] || 0) / total) * 100 : 0;
      if (rate > bestRate) {
        bestRate = rate;
        bestIndex = i;
      }
    });
    return `${dayLabels[bestIndex]} (${Math.round(bestRate)}%)`;
  }
  ...
  function getWorstDay() {
    ...
    return `${dayLabels[worstIndex]} (${Math.round(worstRate)}%)`;
  }
  ...
  function renderTodayView() {
    ...
    container.innerHTML = sorted
      .map((emp) => {
        ...
        const absenceReason = week
          .filter((r) => r.status === "Absent")
          .map(
            (r) =>
              `${r.day}: ${r.leaveReq ? r.leaveReq.reason : "Unrecorded absence"}`,
          )
          .join("; ");
        const weekDisplay = hasRecords
          ? `<span class="att-week-summary">${present} Present, ${absent} Absent</span>${absenceReason ? `<br><span class="att-absence-reason"><i class="bi bi-info-circle"></i> ${absenceReason}</span>` : ""}`
          : `<span class="att-week-summary" style="color: var(--text-muted); font-style: italic;">New employee - no records</span>`;
    ...
    container
      ...
      .forEach((btn) =>
        btn.addEventListener("click", () =>
          showEmployeeDetails(parseInt(btn.dataset.employeeId, 10)),
        ),
      );
  }
  ...
  function renderHistoryView() {
    ...
    if (!container) return;
    if (!employees.length) {
      container.innerHTML = `<tr><td colspan="7" class="text-center text-muted py-4">No employees found.</td></tr>`;
      return;
    }
                ...
                const cls =
                  r.status === "Present"
                    ...
                    : r.status === "Absent"
                      ? "absent"
                      : "";
                return `<td><span class="att-status-cell ${cls}">${label}</span></td>`;
              })
              .join("")
          : `<td colspan="5" style="color:var(--text-muted);font-style:italic;text-align:center;">No attendance records</td>`;
  ...
  function toggleView() {
    const todayView = document.getElementById("attTodayView"),
      historyView = document.getElementById("attHistoryView"),
      toggleBtn = document.getElementById("attToggleHistoryBtn");
    ...
    toggleBtn.innerHTML = showingToday
      ? '<i class="bi bi-arrow-left"></i> Back to Today'
      : '<i class="bi bi-calendar-week"></i> View Full History';
  ...
  function populateEmployeeSelect() {
    ...
    select.innerHTML =
      ...
      employeeInfo
        .map(
          (e) =>
            `<option value="${e.employeeId}">${e.name} (${e.position})</option>`,
        )
        .join("");
  }
  ...
  function exportAttendance() {
    ...
    try {
      const allDates = [
        ...new Set(employees.flatMap((e) => e.attendance.map((a) => a.date))),
      ].sort();
      const summaryRows = employees.map((emp) => {
        const present = emp.attendance.filter(
          (r) => r.status === "Present",
        ).length;
        const absent = emp.attendance.filter(
          (r) => r.status === "Absent",
        ).length;
        return {
          Employee: emp.name,
          ...
          "Total Absent": absent,
          "Attendance Rate":
            present + absent > 0 ? present / (present + absent) : 0,
        };
      });
      ...
      const leaveRows = employees.flatMap((emp) =>
      ...
      const ws3 = XLSX.utils.json_to_sheet(leaveRows);
      ...
      showToast(
        `Attendance report exported for ${employees.length} employees.`,
        "success",
      );
    } catch (error) {
  ...
  document.getElementById("attDate").value = today;
  document
    .getElementById("attSubmitBtn")
    .addEventListener("click", logAttendance);
  document
    .getElementById("attExportBtn")
    .addEventListener("click", exportAttendance);
  document
    .getElementById("attToggleHistoryBtn")
